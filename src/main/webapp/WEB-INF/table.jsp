...
<html>
   ...
   <body>
         ...
         <% 
           ...
           String wastestream = (String) request.getAttribute("wastestream"); 
...
document.addEventListener("click", function(evt) {  
  ...
  <li id="dropdown" class="dropdown" >
       ...
       <ul class="nav dropdown-content" id="dropdown-content">
        ...
        <li>
            <form:form method="POST" commandName="editUser" action="editUsertable" >
  ...
  <form:form method="POST" commandName="commandcg"  action="compgroup">
                              ...
                              <form:select onchange="this.form.submit()" path="compgroup" class=" col-md-10 selectpicker style-select" data-live-search="true"  >
   ...
   <form:form method="POST" commandName="commandcg"  action="compgroup">
                              ...
                              <form:select onchange="this.form.submit()" disabled="true" path="compgroup" class=" col-md-10 selectpicker" data-live-search="true"  >
           ...
           <div class="row  container-row style-6 available" style="margin-top:2%; margin-bottom:1%;" >
            <div>
                ...
                <a class="btn btn-success" disabled href="#" >Available</a>
             </div>
        <div>
            <form:form  method="POST" commandName="commandw" action="table1">
            <form:input class="hide"  path="wastestream" value = "<%=wastestream%>"/>
              <form:button class="btn btn-success" autocomplete="off" path="wastestream" type="submit"> Not Available </form:button>  
                 </form:form>
      ...
      <c:if test="${result1.size() > 0}" >
        <%
            ...
            { Integer size=(Integer) request.getAttribute("result1size");
  ...
  <c:forEach items = "${result1}" var="ws" varStatus="pStatus">
      
              <tr>
  <form:form method="POST" commandname="command" action="editWastevalue">
      <form:input class="hide"  path="valueid" value = "${ws.getValueid()}"/>
           <td class="editn" contenteditable="false"><c:out value = "${ws.getCompgroup()}"/>  </td>
    ...
    
{
        ...
        <div id="table" class="table-editable container" style="text-align:center; margin-top:2%;">
         ...
         <tbody>
             <c:forEach items = "${result1}" var="ws" >
             
              <tr>
       
           <td class="editn" contenteditable="false">${ws.getCompgroup()}</td>
           <td class="editn" contenteditable="false" >${ws.getCftc()}</td>   
           <td class="edit" contenteditable="false" > ${ws.getValue()}</td>
           <td class="edit" contenteditable="false" >${ws.getUftc()}</td>
           <td class="edit" contenteditable="false">${ws.getCitation()}</td>
             ...
             <%}%>
                                   ...
                                   <div class="contrainer text-center" >
                                         <%String value="/table?wastestream="+wastestream+"&foodname="+foodname; %> 
